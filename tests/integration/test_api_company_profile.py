...
async def test_get_company_profile_happy_path():
    ...
    async with SessionLocal() as db_session:
        db_session.add(
            CompanyProfile(
                company_id=company_id, description="A test company", crawled_date=datetime.now(UTC).replace(tzinfo=None)
            )
        )
...
async def test_generate_summary_happy_path():
    ...
    class MockAIClient:
        async def extract_company_summary(self, *args, **kwargs):
            return {"summary": "AI generated summary here.", "is_sufficient_data": True}

    transport = ASGITransport(app=app)
    with patch("core.ai_client.get_ai_client", return_value=MockAIClient()):
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post(f"/api/v1/company/{company_id}/summarize/hiring", json={})
...
async def test_generate_summary_invalid_type():
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        response = await ac.post(f"/api/v1/company/{company_id}/summarize/invalid_type", json={})
...
async def test_generate_summary_ai_failure():
    ...
    with patch("core.ai_client.get_ai_client", return_value=MockAIClient()):
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post(f"/api/v1/company/{company_id}/summarize/financial", json={})
...
async def test_evaluate_historic_tenders_happy_path():
    ...
    class MockAIClient:
        async def evaluate_historic_competition(self, historic_tenders, company_id):
            return {"incumbent_advantage_summary": "Low advantage", "competitor_density_summary": "High density"}
    ...
    with patch("core.ai_client.get_ai_client", return_value=MockAIClient()):
        with patch.object(httpx.AsyncClient, "post", new=mock_post):
            async with AsyncClient(transport=transport, base_url="http://test") as ac:
                response = await ac.post(f"/api/v1/company/{company_id}/historic-tenders")
