...
class BidActivity(Base):
    ...
    id: Mapped[str] = mapped_column(String(128), primary_key=True, default=_uuid)
    bid_id: Mapped[str] = mapped_column(String(128), ForeignKey("bid.id"), index=True)
...
class CompanyNorthData(Base):
    ...
    history_timeline: Mapped[dict | list | None] = mapped_column(JSON)
    persons_timeline: Mapped[dict | list | None] = mapped_column(JSON)
    marketing_tech: Mapped[dict | list | None] = mapped_column(JSON)
    tab_metrics: Mapped[dict | list | None] = mapped_column(JSON)
    network_links: Mapped[dict | list | None] = mapped_column(JSON)
