...
async def get_company_insolvency(company_id: str, db: AsyncSession = Depends(get_db)):
    ...
    stmt = select(CompanyInsolvency).where(
        CompanyInsolvency.company_id == company_id, CompanyInsolvency.crawled_date >= thirty_days_ago
    )
    ...
    cleaned_company_id = re.sub(
        r"\b(GmbH & Co\.? KG|GmbH|AG|UG|haftungsbeschränkt|e\.?V\.?|KG|OHG)\b",
        "",
        cleaned_company_id,
        flags=re.IGNORECASE,
    )
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        try:
            crawling_response = await client.post(f"{CRAWLING_MS_URL}/api/v1/scrape/insolvency", json=payload)
            ...
            logger.warning(
                f"Failed to fetch insolvency data from crawling ms: {e}. Returning cached data if available."
            )
